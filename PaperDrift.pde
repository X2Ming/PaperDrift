...
// 音乐由 AudioManager.java 负责，这里只保存对象和开关。
...
// 纸张纹理提前画到 PGraphics 里，这样运行时不会太卡。
...
void settings() {
  // 全屏是项目要求，smooth 让纸飞机和 UI 边缘更柔和。
  fullScreen();
  smooth(4);
}

void setup() {
  // 固定 60 帧，方便用 frameCount 计算秒数。
  frameRate(60);
...
void startPhaseTwo() {
  // 进入暗黑模式：只设置状态，视觉和速度由 darkBlend 慢慢过渡。
  phase = 2;
  phase2Started = true;
